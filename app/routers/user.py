...
def get_current_user(user_data: User = Depends(user_crud.get_current_user)):
    """
...
@router.get("/{email}", response_model=UserSchema, summary="Get user by email")
def get_user(email: str, db: Session = Depends(get_db), current_user: User=Depends(user_crud.get_current_user)):
    """ "
